...
Prob_dens_sub.fill_between(age_vec[i_sigma_inf:i_sigma_sup],proba_age_vec[i_sigma_inf:i_sigma_sup],alpha=0.2, color='#e07109')
...
for i_Diam in range(0,N_Diam+1):
    Best_isochron_cumu[i_Diam] =  Age_factor*10**np.sum(Coef_PF[1:] * np.log10(Diam_bin[i_Diam])**np.arange(1,len(Coef_PF)))
